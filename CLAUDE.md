...
PART in any key, in treble, bass or tenor clef. It has never been tested on a
piano score, two voices on one stave, or a clef change mid-system.

## The four measurements, in the order they rank
...
2. **RECALL MUST NOT FALL.** Not on any page by more than 0.3, and not on the
   ...
   is cosmetic.
3. **NULL PROPAGATES AND IS NEVER DEFAULTED.** A cello part is in bass clef most
...
4. **DO NOT EDIT `pages/truth/*.json` WITHOUT LOOKING AT THE PAGE.** Those files
